		...
		<c:if test="${deals ne null and  not empty deals}">
				<div class="col-lg-8 col-md-8 col-sm-12 col-xs-12" style="padding:0px;">
					<c:forEach var="deal" items="${deals}" varStatus="status">
						<br/><br/><strong>Deal</strong>: 	id :: ${deal.id} description :: ${deal.description }   validFrom :: ${deal.validFrom } validTill :: ${deal.validTill }<br/>
							<c:forEach var="service" items="${deal.services}" varStatus="status">
								service : ${ service.serviceId} :::: service name ${ service.name} image::${ service.image}<br/>
					</c:forEach>		
					</c:forEach>	
				</div>
			</c:if>
