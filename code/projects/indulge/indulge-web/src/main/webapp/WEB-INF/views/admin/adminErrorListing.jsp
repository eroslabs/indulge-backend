							...
							<c:forEach var="stats" items="${result.response}"
								...
								<tr>
									<c:forEach var="item" items="${stats}" varStatus="statusItem">
										...
										<c:choose><c:when test="${item.key == 'merchantId' }"><a href="${pageContext.servletContext.contextPath }/merchant/homeFromAdmin?id=${item.value}" >${item.value}</a></c:when><c:otherwise>${item.value}</c:otherwise></c:choose></td>
									</c:forEach>
								</tr>
							</c:forEach>
